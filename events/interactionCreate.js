		...
		switch (interaction.type) {
			...
			case 3: // Message Components
				if (interaction.isButton()) {
					...
					if (interaction.customId === enterEmailCustomId) {
						await require('../buttons/enterEmail').execute(interaction);
					} else if (interaction.customId === verifyOTPCustomId) {
						await require('../buttons/verifyOTP').execute(interaction);
					} else if (interaction.customId === supportCustomId) {
						await require('../buttons/support').execute(interaction);
					} else {
			...
			case 5: // Modal submit
				...
				// Handle specific modal submissions
				if (interaction.customId === 'emailModal') {
					await handleEnterEmailModal(interaction);
				} else if (interaction.customId === 'otpModal') {
					await handleVerifyOTPModal(interaction);
				} else {
